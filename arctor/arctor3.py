...
#             n_sig=n_sig, window=window)
        print(self.clean_cosmic_rays_temporal_simple(                                                   #Turned return to print
            n_sig=n_sig, window=window))
...
#        print(xcenters_, ycenters_, positions)                                        #Added print
...
#        print('done 1')                                                              #ADDED PRINT STUFF
        start = time()
        pool = mp.Pool(mp.cpu_count() - 1)
#        print(pool)
#        print('done 2')                                                         #THIS IS WHERE IT STOPS OFTEN
...
#        print('done 4')
        ...
        if 'xcenter' in photometry_df.columns:
            photometry_df.drop(['xcenter'], axis=1, inplace=True)                              #I'm trying a dumb thing
            #photometry_df.drop(['ycenter'], axis=1, inplace=True)
    ...
    def load_data(self, transit, load_filename=None, sort_by_time=False):
...
#                 print('key',key)
                with fits.open(filename) as val:
                    self.fits_dict[key] = val
#                     print('self.fits_dict[key]', self.fits_dict[key])
                    # fits_dict[key] = val
                    header = val['PRIMARY'].header
                    image = val['SCI'].data
#                     image = image[0:,178:220]  #Prob. very wrong (Only looks at 3nd star)
                    image = image[transit[0]:transit[1],transit[2]:transit[3]]
#                    print('image_shape', image.shape)
    ...
    def save_dict(self, save_filename):
        ...
        for key, val in self.__dict__.items():
            if key != 'fits_dict' and not hasattr(val, '__call__'):
                save_dict_[key] = val
